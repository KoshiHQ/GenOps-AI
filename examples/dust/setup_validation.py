...
def main():
    ...
    if api_key and workspace_id:
        ...
        print(f"Explicit validation result: {'✅ PASSED' if explicit_result.is_valid else '❌ FAILED'}")
    ...
    recommendations = generate_setup_recommendations(result if 'result' in locals() else None)
...
def generate_setup_recommendations(result=None):
    ...
    if not os.getenv("GENOPS_TEAM"):
        recommendations.append(
            "Set GENOPS_TEAM for cost attribution: "
