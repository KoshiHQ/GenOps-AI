...
class ComplianceMonitor:
    ...
    def _initialize_policies(self):
        ...
        base_policies = {
            ...
            "authorized_models": {"allowed": ["gpt-3.5-turbo", "gpt-4", "claude-3"], "enabled": False}
        }
        ...
        logger.info(f"Initialized {len(base_policies)} policies for {self.compliance_level.value} compliance")
    ...
    def start_monitoring(self) -> bool:
        ...
        try:
            ...
            success = auto_instrument(
                ...
                audit_enabled=True,
                data_retention_days=30 if self.compliance_level != ComplianceLevel.BASIC else 7
            )
            
            if success:
                self.is_monitoring = True
                logger.info(f"Compliance monitoring started with {self.compliance_level.value} level")
                return True
            else:
    ...
    def _check_policy_compliance(self, event: ComplianceEvent):
        ...
        if self.active_policies.get("daily_budget_limit", {}).get("enabled"):
            daily_spend = sum(e.cost for e in self.audit_events 
    ...
    def _create_violation(
        ...
        logger.warning(f"Policy violation detected: {violation_id} - {violation_type.value}")
    ...
    def get_compliance_report(self, team: Optional[str] = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "unresolved_violations": len([v for v in violations if not v.auto_resolved]),
                "total_cost": sum(e.cost for e in events),
                "unique_users": len(set(e.user_id for e in events if e.user_id)),
                "unique_teams": len(set(e.team for e in events))
            },
    ...
    def _get_cost_breakdown(self, events: List[ComplianceEvent]) -> Dict[str, Any]:
        ...
        for event in events:
            # By team
            breakdown["by_team"][event.team] = breakdown["by_team"].get(event.team, 0) + event.cost
            
            # By project  
            breakdown["by_project"][event.project] = breakdown["by_project"].get(event.project, 0) + event.cost
            
            # By provider
            breakdown["by_provider"][event.provider] = breakdown["by_provider"].get(event.provider, 0) + event.cost
            
            # By customer (if present)
            if event.customer_id:
                breakdown["by_customer"][event.customer_id] = breakdown["by_customer"].get(event.customer_id, 0) + event.cost
        
        return breakdown
    
    def _get_violations_by_type(self, violations: List[PolicyViolation]) -> Dict[str, int]:
    ...
    def _calculate_compliance_score(self, events: List[ComplianceEvent], violations: List[PolicyViolation]) -> float:
...
def demo_policy_enforcement():
    ...
    for i, scenario in enumerate(violation_scenarios):
        ...
        violations = [v for v in monitor.policy_violations if v.violation_type == scenario["expected_violation"]]
        ...
        if violations:
            ...
            print(f"      ✅ No violation detected (unexpected)")
    
    print(f"\n📊 Policy Enforcement Summary:")
    print(f"   Total violations: {len(monitor.policy_violations)}")
    print(f"   Auto-resolved: {len([v for v in monitor.policy_violations if v.auto_resolved])}")
    print(f"   Critical: {len([v for v in monitor.policy_violations if v.severity == 'critical'])}")
...
def demo_gdpr_compliance():
    ...
    for i, scenario in enumerate(gdpr_scenarios):
        ...
        print(f"      • Consent: {scenario['custom_attributes'].get('data_subject_consent', 'unknown')}")
        print(f"      • Lawful basis: {scenario['custom_attributes'].get('gdpr_lawful_basis', 'unknown')}")
        print(f"      • Region: {scenario['custom_attributes'].get('geographic_region', 'unknown')}")
    
    # Show GDPR-specific violations
    gdpr_violations = [v for v in monitor.policy_violations if "pii" in v.policy_name.lower()]
...
def demo_compliance_reporting():
    ...
    reporting_scenarios = [
        ("finance_user", "finance-team", "compliance-reporting", "gpt-4", "openai", 0.030, 1800),
        ("audit_user", "audit-team", "risk-analysis", "claude-3-sonnet", "anthropic", 0.025, 1500),
        ("ops_user", "operations", "process-automation", "gpt-3.5-turbo", "openai", 0.015, 900)
    ]
    ...
    print(f"   Contains full audit trail for regulatory compliance")
