...
class ProviderMetrics:
    ...
    provider: str
    model: str
    latencies: deque = field(default_factory=lambda: deque(maxlen=100))  # Last 100 requests
...
class PerformanceRouter:
    ...
    def get_optimal_provider(
        ...
        for model, provider in equivalent_models:
            ...
            if routing_strategy == "latency":
                # Prioritize latency
                score = 1 / (1 + metrics.avg_latency_ms / 1000) if metrics.latencies else 0.5
    ...
    def clear_expired_cache(self):
        ...
        with self._lock:
            ...
            expired_keys = [
                key for key, cached in self.request_cache.items()
...
class PerformanceOptimizer:
    ...
    def setup_connection_pools(self, pool_size: int = 10):
        ...
        print(f"🔧 Connection pools configured with {pool_size} connections per provider")
    ...
    def optimized_request(
        ...
        optimal_choice = self.router.get_optimal_provider(equivalent_models, routing_strategy)
        ...
        self.router.record_request_result(selected_model, selected_provider, latency_ms, success)
    ...
    def _get_equivalent_models(self, requested_model: str) -> List[Tuple[str, str]]:
        ...
        equivalents = model_equivalents.get(requested_model, [(requested_model, "openai")])
        available = []
    ...
    def benchmark_providers(
        ...
        for model, provider in available_models:
            ...
            if latencies:
                ...
                print(f"   📈 Results: {statistics.mean(latencies):.0f}ms avg, {errors} errors")
...
def check_performance_setup():
    ...
    if len(api_keys_found) < 2:
        ...
        print(f"✅ {len(api_keys_found)} providers configured for performance optimization")
...
def demo_intelligent_routing():
    ...
    test_scenarios = [
        ...
        },
        {
            ...
            "model": "gpt-4",
            "messages": [{"role": "user", "content": "Analyze the market trends and provide insights."}],
            "max_tokens": 200
        }
    ]
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                if result.get("success"):
                    print(f"   ✅ {result['provider_used']}: {result['latency_ms']:.0f}ms")
                else:
    ...
    if summary["providers"]:
        ...
        for key, metrics in summary["providers"].items():
...
def demo_response_caching():
    ...
    for i, query in enumerate(test_queries):
        ...
        try:
            ...
            if result.get("cache_hit"):
                ...
                print(f"   ✅ Cache MISS - {result['latency_ms']:.0f}ms (cached for future)")
    ...
    avg_latency_without_cache = total_latency / (len(test_queries) - cache_hits) if len(test_queries) - cache_hits > 0 else 0
    ...
    print(f"   Performance improvement: ~{avg_latency_without_cache:.0f}ms saved per cached request")
...
def demo_load_balancing():
    ...
    with ThreadPoolExecutor(max_workers=concurrent_workers) as executor:
        ...
        for future in as_completed(future_to_request):
            ...
            if result["success"]:
                print(f"   ✅ Request {result['request_id']:2d}: {result['provider']:10s} ({result['latency_ms']:4.0f}ms)")
            else:
...
def demo_performance_monitoring():
    ...
    if "error" not in benchmark_results:
        ...
        for key, metrics in benchmark_results.items():
            ...
            print(f"      Range: {metrics['min_latency_ms']:.0f}ms - {metrics['max_latency_ms']:.0f}ms")
    ...
    if summary["providers"]:
        ...
        print(f"   🏆 Provider Rankings (by health score):")
        
        for i, (key, metrics) in enumerate(providers_by_health):
            rank_emoji = ["🥇", "🥈", "🥉"][min(i, 2)]
        ...
        for key, metrics in summary["providers"].items():
            ...
            if metrics["avg_latency_ms"] > 3000:  # > 3 seconds
                print(f"   ⚠️  HIGH LATENCY: {metrics['provider']} ({metrics['avg_latency_ms']:.0f}ms)")
                alerts_triggered = True
            
            if metrics["error_rate"] > 0.1:  # > 10% error rate
                print(f"   🚨 HIGH ERROR RATE: {metrics['provider']} ({metrics['error_rate']:.1%})")
                alerts_triggered = True
                
            if metrics["health_score"] < 0.5:  # Health score below 50%
                print(f"   💔 LOW HEALTH SCORE: {metrics['provider']} ({metrics['health_score']:.2f})")
                alerts_triggered = True
        
        if not alerts_triggered:
            print(f"   ✅ All providers operating within normal parameters")
    else:
        print(f"   📈 No performance data available yet")
        print(f"   💡 Make some requests to populate performance metrics")
