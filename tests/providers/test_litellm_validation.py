...
from src.genops.providers.litellm_validation import (
    ValidationStatus,
    ValidationIssue,
    ValidationResult,
...
class TestGenOpsIntegrationValidation:
    ...
    def test_validate_genops_integration_success(self, mock_context, mock_callback):
        ...
        with patch('builtins.__import__') as mock_import:
            ...
            assert any("GenOps LiteLLM provider available" in i.message for i in success_issues)
            assert any("callback system functional" in i.message for i in success_issues)
    
    def test_validate_genops_integration_import_error(self):
        """Test GenOps integration validation when import fails."""
        with patch('builtins.__import__') as mock_import:
            mock_import.side_effect = ImportError("No module named 'genops.providers.litellm'")
    ...
    @patch('src.genops.providers.litellm_validation.GenOpsLiteLLMCallback')
    @patch('src.genops.providers.litellm_validation.LiteLLMGovernanceContext')
    def test_validate_genops_integration_callback_error(self, mock_context, mock_callback):
...
class TestProviderAPIKeyValidation:
    ...
    def test_validate_provider_api_keys_all_configured(self):
        ...
        with patch.dict(os.environ, mock_env, clear=True):
            ...
            successful_providers = [p for p, s in provider_status.items() if s == ValidationStatus.SUCCESS]
    ...
    def test_validate_provider_api_keys_partial_configured(self):
        ...
        with patch.dict(os.environ, mock_env, clear=True):
            ...
            success_issues = [i for i in issues if i.status == ValidationStatus.SUCCESS]
            configured_success = [i for i in success_issues if "configured with" in i.message]
            assert len(configured_success) == 2  # OpenAI and Anthropic
            
            # Should have warnings for unconfigured providers
            warning_issues = [i for i in issues if i.status == ValidationStatus.WARNING]
            unconfigured_warnings = [i for i in warning_issues if "not configured" in i.message]
    ...
    def test_validate_provider_api_keys_none_configured(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            no_providers_error = [i for i in error_issues if "No LLM provider API keys" in i.message]
            assert len(no_providers_error) == 1
            
            # All providers should have warning status
            warning_providers = [p for p, s in provider_status.items() if s == ValidationStatus.WARNING]
    ...
    def test_validate_provider_api_keys_alternate_vars(self):
        ...
        with patch.dict(os.environ, mock_env, clear=True):
            ...
            success_issues = [i for i in issues if i.status == ValidationStatus.SUCCESS]
            google_success = [i for i in success_issues if "Google configured" in i.message]
...
class TestConnectivityValidation:
    ...
    def test_validate_litellm_connectivity_success(self, mock_litellm):
        ...
        assert any("gpt-3.5-turbo mapped to provider openai" in msg for msg in model_messages)
        assert any("claude-3-sonnet mapped to provider anthropic" in msg for msg in model_messages)
...
class TestEnvironmentValidation:
    ...
    def test_validate_environment_configuration_missing_vars(self):
        ...
        warning_issues = [i for i in issues if i.status == ValidationStatus.WARNING]
        missing_vars = [i for i in warning_issues if "Missing environment variable" in i.message]
...
class TestComprehensiveValidation:
    ...
    def test_validate_litellm_setup_comprehensive(
        self, mock_connectivity, mock_env, mock_callbacks, 
        mock_api_keys, mock_genops, mock_installation
    ):
    ...
    def test_validate_litellm_setup_exception_handling(self):
        ...
        with patch('src.genops.providers.litellm_validation.validate_litellm_installation') as mock_install:
...
class TestValidationReporting:
    ...
    def test_print_validation_result_with_errors(self, capsys):
        ...
        result.add_issue("Error Component", ValidationStatus.ERROR, "Something failed", "Fix it")
        result.add_issue("Warning Component", ValidationStatus.WARNING, "Something suspicious")
        result.summary = {
            "total_issues": 2,
            "errors": 1,
            "warnings": 1
