...
class GenOpsDustAdapter:
    ...
    def __init__(self, api_key: Optional[str] = None, workspace_id: Optional[str] = None, base_url: str = "https://dust.tt", 
    ...
    def _make_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"Request to Dust API timed out. The service may be experiencing high load or network issues."
            ...
            if status_code == 401:
                error_msg = f"Authentication failed with Dust API. Verify your DUST_API_KEY is correct and has not expired."
                ...
                error_msg = f"Rate limit exceeded for Dust API. Please retry after a brief delay or contact Dust support to increase limits."
    ...
    def create_conversation(self, **kwargs) -> Any:
        ...
        with self.telemetry.trace_operation(operation_name, **trace_attrs) as span:
            try:
                ...
                response = self._make_request("POST", "conversations", conversation_data)
    ...
    def send_message(self, conversation_id: str, content: str, **kwargs) -> Any:
        ...
        with self.telemetry.trace_operation(operation_name, **trace_attrs) as span:
            try:
                ...
                response = self._make_request("POST", f"conversations/{conversation_id}/messages", message_data)
                ...
                if response and isinstance(response, dict):
                    ...
                    if message:
                        ...
                        if "content" in message and isinstance(message["content"], str):
                            estimated_output_tokens = len(message["content"].split()) * 1.3
    ...
    def run_agent(self, agent_id: str, **kwargs) -> Any:
        ...
        with self.telemetry.trace_operation(operation_name, **trace_attrs) as span:
            try:
                ...
                response = self._make_request("POST", f"agents/{agent_id}/runs", agent_data)
    ...
    def search_datasources(self, query: str, **kwargs) -> Any:
        ...
        with self.telemetry.trace_operation(operation_name, **trace_attrs) as span:
            try:
                ...
                response = self._make_request("POST", "data_sources/search", search_data)
                ...
                if response and isinstance(response, dict):
                    ...
                    for doc in documents:
                        if isinstance(doc, dict) and "chunk" in doc and "text" in doc["chunk"]:
...
def instrument_dust(api_key: Optional[str] = None, workspace_id: Optional[str] = None, **kwargs) -> GenOpsDustAdapter:
    """
...
def auto_instrument(**config) -> bool:
    ...
    try:
                ...
                try:
                    ...
                    try:
                        if response.headers.get('content-type', '').startswith('application/json'):
                            ...
                            if operation_type == "conversation_create" and "conversation" in response_data:
                                span.set_attribute("conversation_id", response_data["conversation"].get("sId", ""))
                            elif operation_type == "message_send" and "message" in response_data:
                                span.set_attribute("message_id", response_data["message"].get("sId", ""))
                                # Estimate tokens from message content
                                content = response_data["message"].get("content", "")
                                if content:
                                    estimated_tokens = len(content.split()) * 1.3
                                    span.set_attribute("tokens_estimated_output", int(estimated_tokens))
                            elif operation_type == "agent_run" and "run" in response_data:
                                run_data = response_data["run"]
                                span.set_attribute("run_id", run_data.get("sId", ""))
                                span.set_attribute("run_status", run_data.get("status", ""))
        ...
        logger.info("   All HTTP requests to dust.tt/api/v1 will be automatically tracked")
        return True
