...
try:
    ...
    logger.warning("LiteLLM not installed - provider available but limited functionality")
...
class GenOpsLiteLLMCallback:
    ...
    def input_callback(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.telemetry:
                ...
                self.telemetry.start_span(span_name, {
            ...
            if (self.governance_manager and 
                self.governance_context.governance_policy == "enforced"):
                current_spend = self._get_daily_spend()
                if current_spend >= self.governance_context.daily_budget_limit:
                    raise Exception(f"Daily budget limit ${self.governance_context.daily_budget_limit} exceeded")
    ...
    def success_callback(self, kwargs: Dict[str, Any], completion_response: Any, start_time: float, end_time: float) -> None:
        """Handle success callback - called after successful LiteLLM request."""
        try:
            # Extract usage and cost information
            usage_stats = self._extract_usage_stats(kwargs, completion_response, start_time, end_time)
            ...
            if self.telemetry:
                ...
                self.telemetry.end_span({
                    "genops.status": "success",
                    "genops.cost.currency": "USD"
                })
            
            # Log cost information
            logger.info(f"LiteLLM request completed: {usage_stats.provider}/{usage_stats.model} "
                       f"cost=${usage_stats.cost:.6f} tokens={usage_stats.total_tokens}")
                       
        except Exception as e:
            logger.debug(f"Error in LiteLLM success callback: {e}")
    
    def failure_callback(self, kwargs: Dict[str, Any], completion_response: Any, start_time: float, end_time: float) -> None:
        ...
        try:
            ...
            if self.telemetry:
                self.telemetry.record_metrics({
                    "genops.latency.ms": latency_ms,
                    "genops.status": "error"
                })
                
                self.telemetry.end_span({
                    "genops.status": "error",
                    "genops.error": str(completion_response) if completion_response else "unknown_error"
                })
            
            logger.warning(f"LiteLLM request failed: {kwargs.get('model', 'unknown')} "
                          f"latency={latency_ms:.1f}ms error={completion_response}")
                          
        except Exception as e:
            logger.debug(f"Error in LiteLLM failure callback: {e}")
    
    def _extract_usage_stats(self, kwargs: Dict[str, Any], response: Any, start_time: float, end_time: float) -> LiteLLMUsageStats:
        ...
        try:
            ...
            if hasattr(response, '_hidden_params') and 'response_cost' in response._hidden_params:
                ...
                cost = self.cost_calculator.calculate_cost(provider, model, input_tokens, output_tokens)
    ...
    def _get_daily_spend(self) -> float:
        ...
        with _stats_lock:
            return float(_usage_stats['total_cost'])
...
def auto_instrument(
    ...
    **kwargs
    ...
    try:
        ...
        logger.info(f"GenOps LiteLLM auto-instrumentation enabled for team={team} project={project}")
        logger.info("All LiteLLM requests will now include governance telemetry across 100+ providers")
...
def track_completion(
    ...
    **attributes
    ...
    context = type('Context', (), {
        ...
        'project': project or _instrumentation_config.get('project', 'default-project'),
        'customer_id': customer_id,
    ...
    try:
        ...
        logger.debug(f"Completion tracked: {context.model} cost=${context.cost} "
...
def get_usage_stats() -> Dict[str, Any]:
    ...
    with _stats_lock:
        ...
        return {
            ...
            'total_cost': float(_usage_stats['total_cost']),
            'total_tokens': total_tokens,
            'provider_usage': {
                provider: {
            ...
            'instrumentation_config': _instrumentation_config.copy() if _instrumentation_config else {}
        }
...
def multi_provider_cost_tracking(
    ...
    if group_by == "provider" and len(result['cost_by_provider']) > 1:
        ...
        result['cost_analysis'] = {
            "cheapest_provider": min(result['cost_by_provider'], key=result['cost_by_provider'].get),
            "most_expensive_provider": max(result['cost_by_provider'], key=result['cost_by_provider'].get),
...
def validate_setup(
    ...
    """
    try:
        from .litellm_validation import validate_litellm_setup, print_validation_result
        
        result = validate_litellm_setup(quick=quick, test_connectivity=test_connectivity)
...
PROVIDER_INFO = {
    ...
    'supported_providers': [
        'openai', 'anthropic', 'azure', 'vertexai', 'bedrock', 'cohere', 
        'huggingface', 'ollama', 'together', 'replicate', 'palm', 'gemini',
        'claude', 'mistral', 'fireworks', 'anyscale', 'deepinfra', 'perplexity'
    ],
    'genops_features': [
